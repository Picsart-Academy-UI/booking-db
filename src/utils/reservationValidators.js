...
const { ReservationSplit, BadRequest, Conflict } = require('./errorResponse');

const {
  checkReservationDates, conflictingReservations, getToday,
  formatDates, divideReservation
} = require('./reservation-helpers');

exports.attachFormattedDates = async (next) => {
  // formatting the dates;
  this.formattedDates = formatDates(this.start_date, this.end_date);
  return next();
};

exports.checkFormattedDates = async (next) => {
...
exports.checkConflictingReservations = async (next) => {
